                            ...
                            <b-form @submit.prevent="handleSubmit(onKeenuSubmit)" autocomplete="off">
                                <b-modal ref="keenu-select-banks-modal" no-close-on-esc no-close-on-backdrop title="Select Bank">
                                    ...
                                    <template #modal-footer>
                                        <form-submit-button :disabled="!keenu.formParams.checksum" @click.native="onKeenuSubmit()" ref="keenu-submit-button" type="submit" class="ml-auto">Proceed to Payment</form-submit-button>
                                    </template>
                                </b-modal>
                            </b-form>
                        </ValidationObserver>
...
<script>
    export default {
        ...
        created: function () {
            this.$axios.get("utilities/payment-methods/" + this.$store.getters.user.business.country_id, {genericApi: true})
            .then(response => {
        ...
        methods: {
            loadPaymentMethodDetails: function (paymentMethod) {
                ...
                if (paymentMethod.title.toLowerCase() === "easypaisa") {
                    ...
                    this.$http.get("pay/keenu/banks",{genericApi: true})
                    .then(response => {
            ...
            onKeenuSelectBank: function () {
                this.$http.get("pay/keenu/generate-secured-hash", {genericApi: true, params: {
                    amount: this.amount,
                    order_id: this.orderReference + this.$store.getters.user.business.id,
